...
class ObservationBuffer:
	...
	def __init__(self, size, obs_shape):
		self.size = size
		# transpose obs_shape from channel last to channel first dim
		if len(obs_shape) == 3 and obs_shape[2] == 3:
			obs_shape_ = (obs_shape[2], obs_shape[0], obs_shape[1])
			obs_shape = obs_shape_
		shape_ = size, *obs_shape
		self.buffer_ = np.zeros(shape_, dtype=np.uint8)
		self.position = 0
		self.buffer_level = 0
...
class TrainerFeatExtAE:
	...
	def __init__(self, model, sampler, lr=0.001, schedule_lr_step=30, schedule_lr_gamma=0.5,\
		...
		self.optimizer = torch.optim.RMSprop(model.parameters(), lr=lr) #NOTE Minecraft/mcaltenv
		#self.loss = torch.nn.MSELoss()
		self.loss = torch.nn.BCELoss() # NOTE Minecraft/mcaltenv
...
def evaluate_agents(agents, env, trials, episodes, goals, swap_points, feature_extractor,\
	device=None, obs_buffer=None, xml_goalelem=None):
	'''
	...
	Arguments:
		...
		xml_goalelem: xml element object used to refer to the the Minecraft entity used to specify
			goal location in a Minecraft environment instance. If this is set, it means that 
			Malmo Minecraft is being used. Otherwise (if None), it means that the McGridenv 
			(a replica of the Minecraft environment instance used in this work. This replica 
			allows for faster experiments as it is light weight and it is easier to run evaluations
			in parallel)
	Return: None
	'''
	...
	for agent in tqdm.tqdm(agents):
		rewards= evaluate_agent(agent, env, trials, episodes, goals, swap_points, feature_extractor,\
			device, obs_buffer, xml_goalelem)
		agent.set_reward(sum(rewards) / trials)
	return

def set_mc_goallocation(xml_goalelem, goal):
	''' Helper method to set a goal location in a Minecraft environment instance used in this
	work'''
...
def evaluate_agent(agent, env, trials, episodes, goals, swap_points, feature_extractor,\
	...
	for trial in np.arange(trials):
		...
		if xml_goalelem is not None:
			# minecraft env is active
			set_mc_goallocation(xml_goalelem, goals[trial][0]) 
		else:
			# mcaltenv is active
			env.set_goal(goals[trial][0])
		env.reset()
		for episode in np.arange(episodes):
			if swap_points is not None and episode in swap_points[trial]:
				...
				if xml_goalelem is not None:
					set_mc_goallocation(xml_goalelem, goals[trial][idx_])
				else:
					env.set_goal(goals[trial][idx_])
...
def run_episode(featextractor, agent, env, return_actions=False, return_observations=False,\
	...
	'''
	ret = env.reset()
	if (isinstance(ret, tuple) or isinstance(ret, list)) and len(ret) == 4:
		obs, reward, done, info = ret # mcaltenv is active
	else:
		obs = ret # minecraft env is active
	...
	while True:
		...
		# tranpose obs from channel last to channel first dim (format expected by conv nn)
		...
		# apply data transformation tot the latent features
